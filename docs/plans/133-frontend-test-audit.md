...
| `src/components/CreateTeam/CreateTeam.test.tsx`               | 165   | **Done (Commit 6): migrate + delete**      | Loader (`requireNoTeam`), submit-success → `/team/$teamId` navigation, submit-failure InlineError, empty-name validation block, and redirect-search-param branch all moved to new `create-team.integration.test.tsx`. File deleted. No `<CreateTeamForm />` extraction — single-purpose form, no reuse pressure. Sign-up → create-team golden path stays owned by `team.spec.ts`.                                                                                                                                                                                   |
...
### Commit 6 — Migrate create-team flow → integration; delete the unit test (done)

**Done:**

- New `web/src/tests/integration/create-team.integration.test.tsx` (5 cases) covering `/create-team` end-to-end: loader renders the form behind the real `requireNoTeam` guard (MSW `GET /me/team` → 404), submit-success POSTs `/teams` and navigates to a stub `/team/$teamId`, submit-failure (POST → 500) surfaces `role="alert"` and stays on the form, empty-name submit blocks with a "Team name is required" field error and no POST fires, and a `?redirect=/leagues` search param navigates to a stub `/leagues` after submit.
- `CreateTeam.test.tsx` deleted. No `<CreateTeamForm />` extraction (single-purpose form, no reuse pressure).
- Split into its own file — not appended to `team-lineup.integration.test.tsx`. The team-lineup file was already 394 lines pre-Commit-6, well past the 250-line split threshold the plan called out.

**What changed vs. the original plan:**

- **Plan said "Submit success → navigates to `/my-team`"; reality navigates to `/team/$teamId`.** That's what `CreateTeam.tsx` actually does in production (`navigate({ to: '/team/$teamId', params: { teamId: String(createdTeam.id) } })`). The original plan was wrong on the destination; the integration test asserts the actual behavior. The route component is identical otherwise.
