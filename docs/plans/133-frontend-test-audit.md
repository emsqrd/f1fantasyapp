...
For each "delete as redundant" or "migrate to integration" call below, the question is whether the journey-level assertions in the original test are already covered by an E2E that *clicks* the right thing (not just visits a page). After reading every E2E spec:
...
**Sequencing note.** Commit 1 (CLAUDE.md doc) should ship first so reviewers read the framing before reviewing migrations. Commits 2–8 don't strictly depend on each other; the order shown groups by feature area for review-friendly diffs.

### Commit 1 — `web/CLAUDE.md`: three-way split rule (docs only)

Add a "Choosing the right layer" subsection under the existing "Frontend Test Layering" section:

- *Asserts on a single unit's behavior given props/state* → jsdom unit/component (existing layer).
- *Asserts on multi-component wiring with a fake backend* → frontend integration (`src/tests/integration/`).
- *Asserts on a user journey through the full composed system* → E2E (`e2e/tests/`).
- *If the setup is longer than the assertions, the test is in the wrong layer.*

No code changes. Sets the framing reviewers will read while reviewing the migration commits.
...
- Two users: A owns a *public* league; B is signed in with a team and no membership.
...
- `DriverPicker.test.tsx`: delete the budget-pool-filtering assertion if it duplicates what the integration test now proves; keep the unit-level "given prop, render correctly" tests.
- `ConstructorPicker.test.tsx`: delete the line-156 "only displays constructors not in current lineup" assertion (now in integration where it can prove the wiring); keep prop-driven rendering, accessibility, read-only-mode tests.

Both pickers retain their hook-wiring and container-accessibility tests per the existing layering doc.
...
- `web/CLAUDE.md` — three-way split rule (commit 1).
...
- `e2e/tests/{auth,avatar,league,team}.spec.ts` — read-only reference for what's already journey-tested in a real browser; nothing migrates *into* these files unless a gap surfaces during a commit (Commits 2 and 3 each append one small case).
...
8. Read final `web/CLAUDE.md` — three-way split rule is concrete enough that a future contributor can pick the right layer without asking.
