...
| `src/components/Team/Team.test.tsx`                           | 514   | **Extract `<TeamView />` + migrate**       | `MyTeamRoute` and `TeamRoute` already share an underlying view, so extracting `<TeamView team races readOnly onAdd onRemove onCaptain />` has independent design value (real reuse, two consumers). UX enforcement (budget cap, picker-disables-chosen, lock-deadline) moves to `team-lineup.integration.test.tsx`. Presentation-given-props assertions (countdown formatting, captain-error placement, owner-name in readOnly) become unit tests of `<TeamView />`. Delete `Team.test.tsx`. Reload-persist + captain-set golden path stays owned by `team.spec.ts`. |
...
### Commit 5 — Extract `<TeamView />` + migrate team-lineup UX enforcement → integration
