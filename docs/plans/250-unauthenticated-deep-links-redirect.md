...
Surfaced by code review after Commits 1–4 landed. The plan body above is left as written — it records what each commit did at the time; these are the deltas between it and the shipped code.

- **The email-confirmation round-trip cannot carry a URL fragment.** Commit 1's reasoning (and its `auth-confirm` test) assumed that `ConfirmEmailNotice` preserving the hash meant a deep link's `#fragment` survives end to end. It does not on the email path: the confirmation template interpolates the destination into `next` unencoded (`api/supabase/templates/confirmation.html`), so a `#` binds to the `/auth/confirm` URL and never reaches `next`. Path and a single query param survive; the fragment does not. A `urlquery`-encoding template fix was written, then reverted — cosmetic gain (no destination uses a fragment today) against an unverifiable change on the signup critical path. Commit 1's `auth-confirm` fragment-preservation test was removed rather than rewritten — it locked a behavior the email path can't deliver, and the navigate-to-`next` happy path is already covered by the route's other tests; the limitation is recorded here and in ADR 007, not asserted as a non-feature. The "returns whole" / "now-preserved fragment" wording in the Decisions and Commit 1 sections holds only for the direct paths (sign-in, instant-session sign-up, already-authed bounce).
- **`safeInternalPath` also rejects `//`-prefixed resolved paths.** Beyond the origin-only check in the Commit 1 snippet, a dot-segment fold like `/..//evil.com` resolves same-origin yet yields a `//evil.com` pathname — protocol-relative if ever reused as an href. The helper now returns `undefined` when the resolved path starts with `//`, and the rejection matrix gained `/..//evil.com` and `/.//evil.com`.
