...
- **`src/components/Home/MyLeaguesList.tsx`** — becomes the data-owner. `const { data, isPending, isError, refetch } = useQuery(standingsQuery)`. Branch: `isPending` → render nothing; `isError` → error (header-less); `data.length === 0` → `JoinLeaguesPrompt` (moved in from Home); else the `My Leagues` heading + `View all →` + rows.
  ...
  - **Error:** reuse `InlineError` (carries `role="alert"`) for the message "We couldn't load your leagues." plus a `Button variant="outline" size="sm"` "Try again" wired to `refetch()`, both inside a `<section className="flex flex-col gap-3">`. No `My Leagues` heading above it, no new component, no full-page `ErrorState`.
...
- **`src/components/Home/Home.tsx`** — drop the `standings` prop from `HomeProps`; replace the standings branch with `{!summary ? <LeaguesNeedTeamNotice/> : <MyLeaguesList/>}`. Remove the now-unused `JoinLeaguesPrompt` import (it moved to `MyLeaguesList`). Lift the inline `ScoreCard` component out to its own file (next bullet) and import it.
- **`src/components/Home/ScoreCard.tsx`** (new) — the `ScoreCard` component moved verbatim out of `Home.tsx` (eyebrow / title / score, em-dash + `pts` formatting), no behavior change. Extracted so its rendering matrix stays unit-testable without a `QueryClient`: once `Home` composes the standings-fetching `MyLeaguesList`, a `Home`-level render of the score cards would otherwise drag a Query provider into Home's unit test.
...
- **`src/components/Home/Home.test.tsx`** — remove the `leagues list`, `no-leagues`, and `score cards` describes plus the identity-header-with-team case (all now covered elsewhere — see below); drop `standings` from `renderHome`. The file is left owning **only** the no-team branch, which mounts no `MyLeaguesList` and so needs no `QueryClient` — no provider is added here. The score-card rendering matrix moves to `ScoreCard.test.tsx`; the header-with-team + score-value coverage moves to the integration "list renders" case. (This supersedes an earlier approach of keeping the summary-present cases here behind a `QueryClientProvider` seeded with `setQueryData(standingsKeys.all, [])` — extracting `ScoreCard` keeps them as cheaper, provider-free unit tests instead.)
- **`src/components/Home/ScoreCard.test.tsx`** (new) — the score-card rendering matrix lifted out of `Home.test.tsx`: eyebrow/title render, thousands-separator + `pts` suffix, and the em-dash / no-`pts` null case. Pure unit test, no `QueryClient`.
