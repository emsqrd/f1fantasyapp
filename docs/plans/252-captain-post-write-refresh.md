...
- **New — captain survives navigation** (the #252 repro): build a two-route tree (`/my-team` + a stub sibling — copy the `buildStubRoute` pattern from `view-team`/`navigation.integration.test.tsx`). On `/my-team`, set a captain; the PUT returns `200`, **but keep the `GET /me/team` handler returning the *pre-captain* team for the whole test** — "stale" must be genuine, or the `onSettled` refetch returns the new captain and the test passes even pre-fix, destroying the repro. Navigate away and back via the returned `router.navigate({ to: ... })`; assert the new captain still renders (the optimistic cache patch survived the unmount/remount). **Confirm it red-fails against current `main` before writing the fix.**
