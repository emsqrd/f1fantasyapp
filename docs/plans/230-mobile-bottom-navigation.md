...
Owns the "current user's avatar" logic so it isn't duplicated across the sidebar footer and the mobile top bar: reads `profile` from route context (`useMatches()` → `__root__` match), layers an uploaded URL pushed via `avatarEvents`, and tracks the image-loading flag. Returns `{ avatarUrl, isLoading, onLoad, onError }`. Lifts the logic at `AppSidebar.tsx:62-78, 148-155` (profile read, uploaded-URL state, `avatarEvents` subscription, loading state). This is the home for the context/subscription logic that `UserAvatar` no longer holds.

### New `web/src/components/UserAvatar/UserAvatar.tsx`

**Pure presentational leaf** — props in, DOM out, no router, no context. Renders the shadcn `Avatar`: shows `avatarUrl` when present, the `CircleUser` glyph fallback when absent, and the loading-spinner overlay when `isLoading`. Props: `avatarUrl?`, `isLoading?`, `onLoad?`, `onError?`, `className?` (size). Callers pair it with `useCurrentAvatar()`.
...
- `useCurrentAvatar.test.ts` (hook): resolves `profile.avatarUrl`; uploaded URL overrides; updates on `avatarEvents` emit; loading flag toggles on `onLoad`/`onError`. Provide `profile` via a route-context wrapper — **not** `vi.mock('@tanstack/react-router')`.
- `UserAvatar.test.tsx` (unit, leaf, **no router**): image when `avatarUrl` present; `CircleUser` fallback when absent; spinner when `isLoading`; `onLoad`/`onError` fire. Pure props in / DOM out.
- `src/tests/integration/account-menu.integration.test.tsx`: the sign-out flow (success + failure), *My Account* navigation, and theme selection — via `renderWithRouter` with the real router, **no `@/router` and no `@tanstack/react-router` mocks**. Assert real outcomes: `signOut` (the injected `vi.fn` auth value) called and landed on `/`; on failure spy `Sentry.captureException` (don't mock the module) and assert the `toast.error` (render `<Toaster/>` or spy `toast.error`); for theme, wrap in the real `next-themes` provider and assert `document.documentElement` reflects the choice. This replaces the account-dropdown cases removed from `AppSidebar.test.tsx`.
...
- **`matchMedia` is not mocked today** (`setupTests.ts`) and Playwright is Desktop-Chrome-only — Commit 3 adds a global `matchMedia` mock (default desktop, per-test override) to drive `useIsMobile`. It becomes load-bearing for layout, not just the sidebar's internal Sheet decision, so it must implement the `change` event listeners `useSyncExternalStore` subscribes to.
