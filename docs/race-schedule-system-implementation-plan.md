...
```csharp
public static SeasonResponse ToResponseModel(this Season season)
{
    ...
    return new SeasonResponse
    {
        ...
        IsCurrent = now >= season.StartDate && now <= season.EndDate
    };
}
```
...
- SeasonService computes `IsCurrent` based on date range, SeasonMapper receives flag
- RaceService computes `currentRaceId` by finding next upcoming race, then passes to mapper
