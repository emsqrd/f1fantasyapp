...
// ─────────────────────────────────────────────────────────────
// Post-it — absolute-positioned sticky note. Draggable: grab it and move it
// out of the way; its dropped position persists per id across reloads.
function DCPostIt({ children, id, top, left, right, bottom, rotate = -2, width = 180 }) {
  const elRef = React.useRef(null);
  const storeKey = id ? 'dc-postit:' + location.pathname + ':' + id : null;
  const [off, setOff] = React.useState(() => {
    if (!storeKey) return { x: 0, y: 0 };
    try {
  ...
  // Keep the live offset in a ref so the NATIVE listener (which closes over a
  // single render) always reads the current value.
  const offRef = React.useRef(off);
  offRef.current = off;

  // The canvas pans via a NATIVE pointerdown listener on the viewport element,
  // which is an ANCESTOR of this note — so a React synthetic stopPropagation
  // fires too late (at the React root, above the viewport). Attaching a native
  // listener on the note itself lets us stop the event during bubbling, before
  // it ever reaches the viewport's pan handler.
  React.useEffect(() => {
    ...
    const onDown = (e) => {
      ...
      e.preventDefault();
      e.stopPropagation(); // native — keeps the canvas from panning
      const scale = el.getBoundingClientRect().width / el.offsetWidth || 1;
      const startX = e.clientX, startY = e.clientY;
      const base = { ...offRef.current };
      setDragging(true);
      const move = (ev) => {
        const next = { x: base.x + (ev.clientX - startX) / scale, y: base.y + (ev.clientY - startY) / scale };
        offRef.current = next;
        setOff(next);
      };
      const up = () => {
        ...
        document.removeEventListener('pointerup', up);
        setDragging(false);
        if (storeKey) { try { localStorage.setItem(storeKey, JSON.stringify(offRef.current)); } catch {} }
      };
      document.addEventListener('pointermove', move);
      document.addEventListener('pointerup', up);
    };
  ...
  return (
    <div
      ...
      style={{
        ...
        transform: `translate(${off.x}px, ${off.y}px) rotate(${dragging ? 0 : rotate}deg) scale(${dragging ? 1.03 : 1})`,
        transition: dragging ? 'box-shadow .12s, transform .06s' : 'box-shadow .15s, transform .15s',
        cursor: dragging ? 'grabbing' : 'grab',
        touchAction: 'none', userSelect: 'none', zIndex: dragging ? 50 : 5,
      }}
    >{children}</div>
  );
}
